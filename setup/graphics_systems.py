...
def run_command(command):
    ...
    if result.returncode != 0:
        print(
            f"Error: Command '{command}' failed with return code {result.returncode}."
        )
...
def setup_shader_standard(shader_standard_path: str):
    ...
    if os.path.isfile(main_py_path):
        ...
        run_command(f"{PYTHON_COMMAND} {main_py_path} -sd assets/shaders -gc -gp")
    else:
...
def setup_batcher(shader_standard_path: str, batcher_path: str):
    ...
    try:
        ...
        print(
            "Files copied and overwritten for standard.py and shader_summary.py in the batcher directory."
        )
    ...
    run_command(
        f"{PYTHON_COMMAND} {batcher_path}/main.py --config-file .all_required_shader_batchers.txt"
    )
...
def compile_shaders():
    ...
    shader_out_dir = os.path.join(
        "assets", "shaders", "out"
    )  # Just for reference, not created
    ...
    run_command(
        f"{PYTHON_COMMAND} {shader_main_path} {shader_src_dir} {shader_out_dir}"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Setup graphics systems for shaders and batchers."
    )
    parser.add_argument(
        "--setup",
    ...
    if not shader_standard_path and (
        args.setup in ["batcher", "shader-cache"] or args.setup is None
    ):
        print(
            "Error: You're using the batcher but you don't have the shader standard submodule, please add this submodule first."
        )
        sys.exit(1)
    ...
    if args.setup == "batcher":
        ...
        if batcher_path:
            ...
            print(
                "'batcher' directory not found in 'src', make sure it exists before running."
            )
            sys.exit(1)
